...
sub setupCurrentMonth()
    ...
    m.top.findNode("monthLabel").text = monthName + " " + m.currentYear.toStr()
...
sub onDBStatusChange()
    m.statusLabel.text = "DB: " + m.dbTask.status
...
sub onGoogleStatusChange()
    m.statusLabel.text = "GOOG: " + m.googleTask.status
...
sub onPhotoUrls()
    if m.dbTask.photoUrls <> invalid and m.dbTask.photoUrls.count() > 0 then
